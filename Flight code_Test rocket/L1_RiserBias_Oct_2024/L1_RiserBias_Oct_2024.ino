...
void setup() {
  ...
  String csvHeader = TADPOLSTATE.csvHeader + "Left Servo PWM," + "Right Servo PWM," + "Target Lat," + "Target Long," + "Goal Angle," + "Goal Command," + "BNO Calibration Sys," + "BNO Calibration Accel," + "BNO Calibration Gryo," + "BNO Calibration Mag";
...
int loopTime = 0; // in millis seconds
...
void loop() {
  ...
  if (TADPOLSTATE.stage == "Main") {
    if (TADPOLSTATE.stateGPS.satellites > 3) {
      ...
      TADPOLSTATE.turnRateDriver(goalAngle, loopTime);
  ...
  String dataString = TADPOLSTATE.getdataString() + String(TADPOLSTATE.left_servo_value) + "," + String(TADPOLSTATE.right_servo_value) + "," + String(targetCoords.x * 10000000) + "," + String(targetCoords.y * 10000000) + "," + String(goalAngle) + "," + String(TADPOLSTATE.goalCommand)
